...
pub fn get_games()->Result<String,Box<dyn std::error::Error>>{
    let json_file_path= Path::new("./json_files/games.json");
...
pub fn get_slot_games(game_type_id:i32)->Result<String,Box<dyn std::error::Error>>{
    ...
    if game_type_id == 1{json_file_path = Path::new("./json_files/RapidWin.json")}
    if game_type_id == 2{json_file_path = Path::new("./json_files/Scratch.json")}
    if game_type_id == 3{json_file_path = Path::new("./json_files/Slots.json")}
    let file = File::open(json_file_path)?;
